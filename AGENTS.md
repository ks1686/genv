...
- `env` / `shell` / `service` / `files` / `completion`
...
Optional `Searchable`, `VersionLister`, `BatchUpgrader`, `OutdatedLister`, and `IndexRefresher` extensions are defined in `internal/adapter/adapter.go`.
...
- `genv upgrade` and `genv updates check` share the tracked-package upgrade planner; index-based managers refresh once (`IndexRefresher.PlanRefresh`) before outdated detection (`Filters.All` false / `OutdatedLister`). `--all` still refreshes. A failed refresh or ListOutdated keeps that manager's packages rather than silently skipping them. `OutdatedLister` covers brew/linuxbrew, mas, bun, npm/pnpm/yarn, uv/pipx, pip-user, volta, cargo, winget/scoop/choco, pacman/paru/yay, apt/dnf/apk, snap, and vscode (prefers `cursor` on PATH, else `code`; newest stable; pre-release gallery versions are ignored because `--install-extension --force` cannot install them); mas also implements `BatchUpgrader`. `genv upgrade` then runs named system/firmware steps (`internal/upgrade` runner). On Windows the system step uses WUA COM via PowerShell: map ResultCodes to readable errors, treat SucceededWithErrors as soft success unless a per-update result is Failed, document elevation (do not auto-elevate). `genv updates check` / the timer / `updates.autoApply` stay tracked-packages-only and must not grow those OS/firmware steps. `__run-once` is non-interactive: Unix refresh/apply uses `sudo -n`; unelevated Windows skips mutating winget/choco (and paru/yay) instead of UAC. Elevation skips are logged, not apply errors. Interactive `apply`/`upgrade` may still sudo or UAC; system-scope external installs elevate the staging write on that path and refuse elevation unattended. Follow-up slices add rustup, editors, containers, and other extra tools to `upgrade` only.
...
- `updates start` registers the hourly checker via `internal/selfpath.PreferStable`: prefer same-inode PATH hits, else derive Homebrew `<prefix>/bin/<name>` from Caskroom/Cellar versioned paths even when the brew symlink is missing/dangling mid-upgrade. Scheduled PATH strips Homebrew shims; on Windows it keeps scoop/winget shims and appends user shim dirs. `__run-once` has a wall-clock deadline (launchd `TimeOut` / systemd `TimeoutStartSec` / Task Scheduler `ExecutionTimeLimit`) so TLS/keychain hangs cannot wedge StartInterval. Windows uses a per-user `schtasks` task (logon + interval) started through the scheduler so OpenSSH job objects cannot kill the checker. The task XML sets `UserId` under `Principal` and `LogonTrigger` for the current account (`os/user.Current()` with `USERDOMAIN`/`USERNAME` fallback); keep `InteractiveToken` + `LeastPrivilege` (do not switch to `HighestAvailable` or a service account) so unelevated registration remains the supported path and desktop notifications still work. The Exec action is windowless `wscript.exe` plus a `.vbs` `WshShell.Run` style-0 wrapper around the existing `.cmd` — do not make `genv.exe` a GUI subsystem binary. Access-denied creates should surface a short elevated/GPO/foreign-owned-task hint. Heal with `genv updates start`; `updates status` / `validate` warn on dangling agent binaries.
- Unresolved `genv apply` file mismatches skip post-apply hooks with an explicit message; human-readable output names mismatch paths and includes the files plan. `--force` / `--backup` overwrite mismatched managed files. Per-entry `backup: true` replaces that entry's mismatched regular file without `--force`. `genv files adopt <target>` seeds a missing source from the live file, backs the live file up, links it, and records the lock. Lifecycle hooks inherit `os.Stdin` (like package actions) and receive `GENV_YES=true|false` when `--yes` is set; hooks must still opt into manager noninteractive flags (e.g. pacman `--noconfirm`) themselves — genv does not rewrite hook argv.
