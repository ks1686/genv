...
- `updates start` registers the hourly checker via `internal/selfpath.PreferStable`: prefer same-inode PATH hits, else derive Homebrew `<prefix>/bin/<name>` from Caskroom/Cellar versioned paths even when the brew symlink is missing/dangling mid-upgrade. Scheduled PATH strips Homebrew shims; `__run-once` has a wall-clock deadline (launchd `TimeOut` / systemd `TimeoutStartSec`) so TLS/keychain hangs cannot wedge StartInterval. Heal with `genv updates start`; `updates status` / `validate` warn on dangling agent binaries.
- Unresolved `genv apply` file mismatches skip post-apply hooks with an explicit message; human-readable output names mismatch paths and includes the files plan. `--force` / `--backup` overwrite mismatched managed files.
- `genv add` soft-locks the chosen manager via persisted `prefer` (honored first when available, then `managers` map / defaults). Today it can still write the spec before install succeeds and exit 0 on failure; fixing that ordering is a known v4 correctness goal.
