...
_genv() {
	...
	for i in "${COMP_WORDS[@]}"; do
		case "${i}" in
		add | remove | rm | adopt | disown | list | ls | apply | edit | clean | scan | status | completion | validate | upgrade | updates | export | map | pull | init | env | shell | service | profile | version | help)
			cmd="${i}"
			break
			;;
		esac
	...
	if [[ -z "${cmd}" ]]; then
		...
		cmds="add remove rm adopt disown list ls apply edit clean scan status completion validate upgrade updates export map pull init env shell service profile version help"
